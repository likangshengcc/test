	...
	while (key != 'q') 
	{
		gettimeofday(&start1,NULL);	
	   	...
	   	if (zed.grab(runtime_parameters) == ERROR_CODE::SUCCESS) {
    		...
    		for (int i = 0; i < sdl_point_nogrand_set.size(); i++)
    		{
				if((sdl_point_nogrand_set[i].sdl_point_value.x < 1.2)&&(sdl_point_nogrand_set[i].sdl_point_value.x > -1.0)&&(sdl_point_nogrand_set[i].sdl_point_value.y < 	
		                     -0.1)&&(sdl_point_nogrand_set[i].sdl_point_value.y > -1.3))
    		...
    		for (int i = 0; i < restrain_cnt; i++)
   			{
				...
				for (int i = 0; i < sdl_trans_cloud.size(); i++)
				{
					//sdl_image.at<uchar>(sdl_maxz_trans_cloud[i].y, sdl_maxz_trans_cloud[i].x) = 0;
