	...
	while (key != 'q') 
	{
		gettimeofday(&start1,NULL);	
	   	...
	   	if (zed.grab(runtime_parameters) == ERROR_CODE::SUCCESS) {
    		...
    		for (int i = 0; i < sdl_point_nogrand_set.size(); i++)
    		{
				if((sdl_point_nogrand_set[i].sdl_point_value.x < 2.5)&&(sdl_point_nogrand_set[i].sdl_point_value.x > -2.55)&&(sdl_point_nogrand_set[i].sdl_point_value.y < 	
		                     0.8)&&(sdl_point_nogrand_set[i].sdl_point_value.y > -1.3))
			...
			float init_std = 1.2;
			float center_x = 0.0;
    		int transfer_cnt = 10, restrain_cnt = 2;
