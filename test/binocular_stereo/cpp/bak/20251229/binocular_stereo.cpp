		...
		/*	gettimeofday(&end1,NULL);
    		...
    		for (int i = 0; i < sdl_point_nogrand_set.size(); i++)
    		{
				...
				if((sdl_point_nogrand_set[i].sdl_point_value.x < 1.5)&&(sdl_point_nogrand_set[i].sdl_point_value.x > -1.0)&&(sdl_point_nogrand_set[i].sdl_point_value.y < 	
		        	0.05)&&(sdl_point_nogrand_set[i].sdl_point_value.y > -1.5))
			...
			int fd;
			char str[100];
			fd = open("0.csv",O_RDWR|O_CREAT|O_TRUNC,S_IRWXU);
			if(fd < 0){
				printf("\nFail to open File\n");
				return 0;
			}
			...
			for (int i = 0; i < sdl_cloud.size(); i++)
			{
				sprintf(str,"%f,",sdl_cloud[i].sdl_point_value.x);
				write(fd,str,strlen(str));
				sprintf(str,"%f\n",(-1)*sdl_cloud[i].sdl_point_value.y);
				write(fd,str,strlen(str));
			...
			sprintf(str,"%d.csv",j+1);
			fd = open(str,O_RDWR|O_CREAT|O_TRUNC,S_IRWXU);
			if(fd < 0){
				printf("\nFail to open File\n");
				return 0;
			}
			...
			for (int i = 0; i < sdl_point_trans_set.size(); i++)
			{
				...
				write(fd,str,strlen(str));
				sprintf(str,"%f\n",(-1)*sdl_point_trans_set[i].sdl_point_value.y);
				write(fd,str,strlen(str));
			}
    		...
    		for (int i = 0; i < restrain_cnt; i++)
   			{
				...
				printf("%d:sdl_point_trans_set.size()=%d,std_x=%f\n",i,sdl_point_trans_set.size(),std_x); 
				for (int i = 0; i < sdl_point_trans_set.size(); i++)
        		{            
            		sd_res_set.push_back(sdl_point_trans_set[i]);
        		}
				...
				printf("%d:sd_res_set.size()=%d,std_x=%f,center_x=%f\n",i,sd_res_set.size(),std_x,center_x); 
				sdl_point_trans_set.clear();
				for (int i = 0; i < sd_res_set.size(); i++)
        		{            		
            		if (fabs(sd_res_set[i].sdl_point_value.x - center_x) <= std_x) 
                		sdl_point_trans_set.push_back(sd_res_set[i]);
        		}   
