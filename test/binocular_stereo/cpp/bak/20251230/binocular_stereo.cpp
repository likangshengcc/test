	...
	while (key != 'q') 
	{
		...
		/*	gettimeofday(&end1,NULL);
    		...
    		for (int i = 0; i < sdl_point_nogrand_set.size(); i++)
    		{
				...
				if((sdl_point_nogrand_set[i].sdl_point_value.x < 0.8)&&(sdl_point_nogrand_set[i].sdl_point_value.x > -0.2)&&(sdl_point_nogrand_set[i].sdl_point_value.y < 	
			...
			for (int i = 0; i < sdl_cloud.size(); i++)
			{
				sum_init_x += sdl_cloud[i].sdl_point_value.x;
			}
			center_x = sum_init_x /(float)sdl_cloud.size();
			...
			memset(savejpg_name,'\0',sizeof(savejpg_name));
			strcpy(savejpg_name, saveimg_dir);			
			sprintf(str,"/transfer_%d_%ld.csv",j,save_count);
			strcat(savejpg_name,str);
			fd = open(savejpg_name,O_RDWR|O_CREAT|O_TRUNC,S_IRWXU);
			if(fd < 0){
				printf("\nFail to open File\n");
