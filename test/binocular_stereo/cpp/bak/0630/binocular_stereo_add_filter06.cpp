	   	...
	   	if (zed.grab(runtime_parameters) == ERROR_CODE::SUCCESS) {
			...
			if (bdrawmerged && maxitemnum == 3 && vecitem.size() > 0)
			{
				...
				for (auto it = mergearr.begin(); it != mergearr.end(); ++it)
				{
					...
					if(!first_frame)
					{
						...
						switch(it->n){
							case 0:
									...
									if(leftdis.size() >= 3)
									{
										predict_second(leftdis, leftoutput, left_output_v, left_output_a, 0.6, 3, 3, run_timeuse);
							...
							case 1:
									...
									if(frontdis.size() >= 3)
									{
										predict_second(frontdis, frontoutput, front_output_v, front_output_a, 0.6, 3, 3, run_timeuse);
							...
							case 2:
									...
									if(rightdis.size() >= 3)
									{
										predict_second(rightdis, rightoutput, right_output_v, right_output_a, 0.6, 3, 3, run_timeuse);
					...
					switch(it->n){
						case 0:
								cv::putText(image_ocv, msg, cv::Point(50 , 50), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255));
								cv::putText(image_ocv, msg1, cv::Point(50, 70), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255));
								cv::putText(image_ocv, msg2, cv::Point(50, 90), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255));
								break;
						case 1:
								cv::putText(image_ocv, msg, cv::Point(380 , 50), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255));
								cv::putText(image_ocv, msg1, cv::Point(380, 70), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255));
								cv::putText(image_ocv, msg2, cv::Point(380, 90), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255));
								break;
						case 2:
								cv::putText(image_ocv, msg, cv::Point(700 , 50), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255));
								cv::putText(image_ocv, msg1, cv::Point(700, 70), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255));
								cv::putText(image_ocv, msg2, cv::Point(700, 90), cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 0, 255));
								break;
						default:
								break;
					}
