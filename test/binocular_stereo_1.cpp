...
//所有涉及使用new生成的指针、矩阵等需要占用堆栈的变量均要做销毁处理。（未完全优化）
	...
	struct ITEMTRAJ l_item,f_item,r_item;
	   	...
	   	if (zed.grab(runtime_parameters) == ERROR_CODE::SUCCESS) {
			...
			for (int i = 0; i < new_width; i++)
			{
				for (int j = 0; j < new_height; j++)
				{
					...
					if(std::isfinite(point_cloud_value.z))
					{
						...
						{
							if (Y[i][j] < 0.02&&Y[i][j]>-1)//去除地面，地面在视场的远方会出现在较高的位置造成干扰，只保留地面20cm以上的信息。
							{
			...
			for (auto it = vecnoit.begin(); it != vecnoit.end(); ++it)
			{
				...
				//	run_timeuse /= 1000;
					...
					item.n = it->n;
					item.deep = mindeep;
					item.alpha = atan(minzuoyou/mindeep) * 180 / PI;
			...
			if (bdrawmerged && maxitemnum == 3 && vecitem.size() > 0)
			{
				...
				for (auto it = mergearr.begin(); it != mergearr.end(); ++it)
				{
					...
					if(!first_frame)
					{
						...
						switch(it->n){
							case 0:
									...
									/*if(speedl_count > 18)
										speedl_count = 0;
									leftdis.push_back(speed_left[speedl_count++]);
									...
									if(leftdis.size() >= 3)
									{
										predict_second(leftdis, leftoutput, left_output_v, left_output_a, 0.3, 3, 3, run_timeuse);
										//predict_second(leftdis, leftoutput, left_output_v, left_output_a, 0.6, 3, 3, 110);
										it->spd = left_output_v.back();
										//printf("\nleft speed it->spd : %f\n",it->spd);
									...
									/*printf("\nleft speed : \n"); 
									for(int i = 0; i < left_output_v.size(); i++)
										printf("%f,",left_output_v[i]);
									printf("\n");
									printf("\nleft acc speed : \n"); 
									for(int i = 0; i < left_output_a.size(); i++)
										printf("%f,",left_output_a[i]);
									printf("\n");*/

									if(left_output_v.size() > 4)
									{
										k = 0;
										for(auto itt = left_output_v.begin(); k < 4 ; ++k)
											left_output_v.erase(itt);
									}
									if(left_output_a.size() > 0)
									{
										k = 0;
										for(auto itt = left_output_a.begin(); k < 4 ; ++k)
							...
							case 1:
									...
									/*if(speedf_count > 22)
										speedf_count = 0;
									frontdis.push_back(speed_front[speedf_count++]);
									...
									if(frontdis.size() >= 3)
									{
										predict_second(frontdis, frontoutput, front_output_v, front_output_a, 0.3, 3, 3, run_timeuse);
										//predict_second(frontdis, frontoutput, front_output_v, front_output_a, 0.6, 3, 3, 110);
										it->spd = front_output_v.back();
									...
									for(int i = 0; i < front_output_v.size(); i++)
										printf("%f,",front_output_v[i]);
									printf("\n");
									printf("\nfront acc speed : \n"); 
									for(int i = 0; i < front_output_a.size(); i++)
										printf("%f,",front_output_a[i]);
									printf("\n");*/

									if(front_output_v.size() > 4)
									{
										k = 0;
										for(auto itt = front_output_v.begin(); k < 4 ; ++k)
											front_output_v.erase(itt);
									}
									if(front_output_a.size() > 0)
									{
										k = 0;
										for(auto itt = front_output_a.begin(); k < 4 ; ++k)
							...
							case 2:
									...
									/*if(speedr_count > 10)
										speedr_count = 0;
									rightdis.push_back(speed_right[speedr_count++]);
									...
									if(rightdis.size() >= 3)
									{
										predict_second(rightdis, rightoutput, right_output_v, right_output_a, 0.3, 3, 3, run_timeuse);
										//predict_second(rightdis, rightoutput, right_output_v, right_output_a, 0.6, 3, 3, 110);
										it->spd = right_output_v.back();
									...
									/*printf("\nright speed : \n"); 
									for(int i = 0; i < right_output_v.size(); i++)
										printf("%f,",right_output_v[i]);
									printf("\n");
									printf("\nright acc speed : \n"); 
									for(int i = 0; i < right_output_a.size(); i++)
										printf("%f,",right_output_a[i]);
									printf("\n");*/

	
									if(right_output_v.size() > 4)
									{
										k = 0;
										for(auto itt = right_output_v.begin(); k < 4 ; ++k)
											right_output_v.erase(itt);
									}
									if(right_output_a.size() > 0)
									{
										k = 0;
										for(auto itt = right_output_a.begin(); k < 4 ; ++k)
					...
					sprintf(msg1, "offset:%.3fm alpha:%.2f", it->zuoyou,it->alpha);
					...
					//cv::putText(image_ocv, msg, cv::Point(it->pt1.x , it->pt1.y - 40), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 0));
					//cv::putText(image_ocv, msg1, cv::Point(it->pt1.x, it->pt1.y - 25), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 0));
					//cv::putText(image_ocv, msg2, cv::Point(it->pt1.x, it->pt1.y - 15), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 0));

					switch(it->n){
						case 0:
								if(l_item.alpha == -100){
									l_item.alpha = it->alpha;
								}
								else{
									if(it->alpha > l_item.alpha)
										l_item.alpha = it->alpha;
								}			
								cv::putText(image_ocv, msg, cv::Point(50 , 480), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0));
								cv::putText(image_ocv, msg1, cv::Point(50, 500), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0));
								cv::putText(image_ocv, msg2, cv::Point(50, 520), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0));
								break;
						case 1:
								if(f_item.alpha == -100){
									f_item.alpha = it->alpha;
								}
								else{
									if(abs(it->alpha) < abs(l_item.alpha))
										l_item.alpha = it->alpha;
								}
								if(stop_flag){
									cv::putText(image_ocv, msg, cv::Point(380 , 50), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 255));
									cv::putText(image_ocv, msg1, cv::Point(380, 70), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 255));
									cv::putText(image_ocv, msg2, cv::Point(380, 90), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 0, 255));
								}
								else{
									cv::putText(image_ocv, msg, cv::Point(380 , 480), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0));
									cv::putText(image_ocv, msg1, cv::Point(380, 500), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0));
									cv::putText(image_ocv, msg2, cv::Point(380, 520), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0));
								}
								break;
						case 2:
								if(r_item.alpha == -100){
									r_item.alpha = it->alpha;
								}
								else{
									if(it->alpha > r_item.alpha)
										r_item.alpha = it->alpha;
								}
								cv::putText(image_ocv, msg, cv::Point(700 , 480), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0));
								cv::putText(image_ocv, msg1, cv::Point(700, 500), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0));
								cv::putText(image_ocv, msg2, cv::Point(700, 520), cv::FONT_HERSHEY_SIMPLEX, 0.6, cv::Scalar(0, 255, 0));
								break;
						default:
								break;
					}
					if(stop_flag)
						cv::putText(image_ocv, msg3, cv::Point(400 , 20), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 0));
					else
						cv::putText(image_ocv, msg3, cv::Point(400 , 460), cv::FONT_HERSHEY_SIMPLEX, 0.5, cv::Scalar(255, 0, 0));
				}
			}
			else
			{
			...
			if(stop_flag)
			{
				...
				//	resize(image_copy, image_copy, cv::Size(600, 400));
					...
					cv::rectangle(src1, cv::Rect(410, 300, 150, 240), cv::Scalar(255, 0, 0), 2);
					//cv::imshow("2", src1);
					src = src1(cv::Rect(411, 310, 148, 228));
					...
					cv::threshold(gray, threshold_output, threshodsize, 255, cv::THRESH_BINARY);
				...
				//	printf("\nup=%d,down=%d",up,down);
					...
					cv::rectangle(image_ocv, cv::Rect(410, 300, 150, 240), cv::Scalar(255, 0, 0), 2);
			...
			UDP_DATA send_data;
							send_data.ldis = 40.0;
							send_data.loffset = 40.0;
							send_data.lspeed = 0.0;
							send_data.fdis = 40.0;
							send_data.foffset = 40.0;
							send_data.fspeed = 0.0;
							send_data.rdis = 40.0;
							send_data.roffset = 40.0;
							send_data.rspeed = 0.0;
			...
			for (auto it = mergearr.begin(); it!= mergearr.end() && start < maxitemnum;++it, ++start)
			{
				...
				switch(it->n){
					case 0:
							send_data.ldis = it->dis;
							send_data.loffset = it->zuoyou;
							send_data.lspeed = it->spd;
							break;
					case 1:
							send_data.fdis = it->dis;
							send_data.foffset = it->zuoyou;
							send_data.fspeed = it->spd;
							break;							
					case 2:
							send_data.rdis = it->dis;
							send_data.roffset = it->zuoyou;
							send_data.rspeed = it->spd;
							break;
				}								
			...
			if(stop_flag)
			{
				...
				send_data.stopdis = stop_dis;
				//strcat(send_buf,msg);
			}
			else
			{
				send_data.stopdis = 0.0;
			}

			memset(send_data.fn,'\0',sizeof(send_data.fn));
			strcpy(send_data.fn,savejpg_name);
			memcpy(send_buf,&send_data,sizeof(UDP_DATA));
			
			send_num = sendto(sock_fd,send_buf,sizeof(UDP_DATA),0,(struct sockaddr*)&addr_serv,dest_len);	
